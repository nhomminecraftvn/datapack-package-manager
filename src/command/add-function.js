...
module.exports = function addFunction() {
    ...
    try {
        if ((pack_name && pack_data && function_name) && !isNaN(pack_formatRange)) {
          ...
          else if (function_name == undefined) throw `Missing <function_name>`;
          else if (pack_formatRange == undefined) throw `Missing <pack_formatRange>`;
          else if (isNaN(pack_formatRange)) throw `"${pack_formatRange}" it's not a number`
    } catch (expection) {
